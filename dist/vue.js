...
(function (
  ...
  typeof exports === 'object' && typeof module !== 'undefined' // cjs模式
    ? (module.exports = factory())
    : typeof define === 'function' && define.amd // amd异步模块
    ? define(factory)
    : ((global = global || self), (global.Vue = factory())); // window模式
})(this, function () {
  'use strict';
  ...
  var emptyObject = Object.freeze({}); // 冻结对象，无法再被更改
  ...
  function isPlainObject(obj) {
    // 是否是【纯粹对象】。  即 {}、new Object()、Object.create(null)产生的对象。（也就是  Object基类 ）
  ...
  function isValidArrayIndex(val) {
    // 判断是否【可靠数组索引】。 1.大于等于0    2.是整数     3.是有限数。       ————————    从以上几点，可知【自然数】
  ...
  function toString(val) {
    // 转化为为string。    1.Array、【基础对象】，用JSON.stringfy    2.其它，直接调用String方法
    return val == null
      ? ''
  ...
  function makeMap(
    str,
    expectsLowerCase // 是否将传入字符串，全部转化为小写
  ) {
    var map  = Object.create(null); // 空对象
    var list = str.split(','); // 逗号分割法，切割字符串
    for (var i = 0; i < list.length; i++) {
      map[list[i]] = true; // map的key为字符串，value为true
    }
   ...
   */
  var isBuiltInTag = makeMap('slot,component', true); // 生成函数：内置tag

  /**
   * Check if an attribute is a reserved attribute.
   */
  var isReservedAttribute = makeMap('key,ref,slot,slot-scope,is'); // 生成函数，保留属性
   ...
   */
  function cached(fn) {
    var cache = Object.create(null); // 空对象
    return function cachedFn(str) {
      var hit = cache[str]; // 传入参数key，是否存在
      return hit || (cache[str] = fn(str)); // 不存在时，从fn中找；并记录缓存。  下次，就可以直接从cache中拿了。
    };
  }
   ...
   */
  var camelizeRE = /-(\w)/g; // 连字符（非第一个）的形式  的正则
  var camelize   = cached(function (str) {
    // 生成函数，连字符（非第一个） 转 大写。        结果是【驼峰命名法】
    return str.replace(camelizeRE, function (
      _, // 是否匹配中
      c // 第一个元素
    ) {
      return c ? c.toUpperCase() : ''; // 找到了第一个元素，转为大写。    找不到，空字符串。       （因为是单个字母，所以）
    });
  });
   ...
   */
  var hyphenateRE = /\B([A-Z])/g; // 正则，【前面非空格】+【大写字母】
  var hyphenate   = cached(function (str) {
    // 生成函数。        【驼峰命名法】转连字符。
    /**
     * -$1，【-号】加上【匹配的第一个元素】
     */
    return str.replace(hyphenateRE, '-$1').toLowerCase(); // 将【前面非空格】+【大写字母】，转化为【小写字母】。
  });
  ...
  function polyfillBind(                                  // 兼容化的bind
    fn,         // 函数
    ctx,        // 调用函数的对象
  ) {
    function boundFn(
      ...
      var l = arguments.length;       // 传入参数个数
      return l
        ? l > 1
          ? fn.apply(ctx, arguments)    // 尽量不使用...解构符
          : fn.call(ctx, a)             // 尽量不使用...解构符
        : fn.call(ctx);   // 无参数
    }

    boundFn._length = fn.length;    // 同时，也是一个数组？？？？？？
    return boundFn;
  }

  function nativeBind(fn, ctx) {                          // 本地bind
    return fn.bind(ctx);
  }

  var bind = Function.prototype.bind ? nativeBind : polyfillBind;   // 当bind未实现时，我们用    【apply / call】 来进行模拟。

  /**
   * Convert an Array-like object to a real Array.
   */
  function toArray(list, start) {       // 将对象，转化为【数组】。
    start   = start || 0;
    var i   = list.length - start;    // 这里，不比较两者，大小关系吗？
    var ret = new Array(i);
    while (i--) {
      ret[i] = list[i + start];
    }
   ...
   */
  function extend(to, _from) {        // 将 from 的所有属性，都赋值到 to 的属性之上。
    for (var key in _from) {    // 特别注意：此处，字符串作为【类数组】来处理。
      to[key] = _from[key];
    }
    return to;
  }
   ...
   */
  function toObject(arr) {            // 把【数组】中的每个元素，都合并到一个数组中去。
    var res = {};     // 空对象。
   ...
   */
  function noop(a, b, c) {        // 为了适配Flow的无意义操作。
  }

  /**
   * Always return false.
   */
  var no = function (a, b, c) {   // 永远返回 false 。
    return false;
  };
   ...
   */
  var identity = function (_) {   // 返回自身。（如果是引用对象，返回自身引用）
    return _;
  };
   ...
   */
  function genStaticKeys(modules) {                   // 具体作用是？？？？？？
    return modules
      .reduce(function (keys, m) {            // 将 modules数组 的每个元素的 staticKeys ，拼接为一个新数组。（类似于 Array.prototype.map ）
   ...
   */
  function looseEqual(a, b) {                               // 形似
    if (a === b) {    // 引用相等
      return true;
    }
    var isObjectA = isObject(a);    // 都是【Object基类】对象
    var isObjectB = isObject(b);    // 都是【Object基类】对象
    if (isObjectA && isObjectB) {
      try {
        var isArrayA = Array.isArray(a);
        var isArrayB = Array.isArray(b);
        if (isArrayA && isArrayB) {         // 是否都是 数组？
          return (
            a.length === b.length &&      // 长度相等
            a.every(function (e, i) {     // Array.prototype.every。子项是否每一个，都满足特定条件。
              return looseEqual(e, b[i]); // 递归
            })
          );
        } else if (a instanceof Date && b instanceof Date) {  // 是否都是 日期？
          return a.getTime() === b.getTime(); // 时间戳相等
        } else if (!isArrayA && !isArrayB) {                  // 都不是数组
          var keysA = Object.keys(a);         // A的keys
          var keysB = Object.keys(b);         // B的keys
          return (
            keysA.length === keysB.length &&  // keys长度相等
            keysA.every(function (key) {      // ...
              return looseEqual(a[key], b[key]);  // 递归。所有键值，是否【形似】。
            })
          );
        } else {
          /* istanbul ignore next */
    ...
    } else if (!isObjectA && !isObjectB) {    // 两个都不是【Object基类】对象
      return String(a) === String(b);   // 转化为字符串后，比较
    } else {
      return false;   // 一个是【Object基类】，一个不是【Object基类】；明显不是【形似】。
    }
  }
   ...
   */
  function looseIndexOf(arr, val) {         // 在一个数组中，找与指定对象，【形似】的索引。
    for (var i = 0; i < arr.length; i++) {
